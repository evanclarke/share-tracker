...
After X, the next SCENARIOS pass is section **Y. Web UI** (12 scenarios), driven the way S through X
were: run every scenario against a throwaway database, apply the standing probes to each, and log
what each raises as a `## SCENARIOS Y-nn` section here with the option Evan chose. The lessons worth
carrying forward are in the handover memory. X added two, and both are about *verification* rather
than about the fix. First, **verify a fix at a scale it was not measured at**: X-b exists only
because X-a's fix was re-run against a database four times larger than the one that found it — at the
original scale the concurrent write simply waits and succeeds, and nothing looks wrong. Second,
**two contradictory measurements can both be right, so re-run a reproduction on the artefact that
produced it before dismissing it**: the pass fixing X-b could not reproduce its headline 6.46 s
(measuring 0.55–2.30 s at the same 30,000 parcels) and recorded that the reproduction did not fail on
this machine; re-run against the database the finding was raised on it reproduced three times running
at 6.5 s. What differed was the database, not the machine — snapshot generation's cost tracks
everything the three reports walk, spanning 18 µs to 218 µs per parcel at one parcel count, which is
why the new timeout is sized on the worst rate seen rather than the typical one.
