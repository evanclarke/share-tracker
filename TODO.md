...
- [x] Avoid CAST(REAL AS TEXT) float imprecision for any rows written before migration 0006 (trades/income were created as REAL in 0004/0005) — `db::canonicalize_pre0006_decimals` runs once at startup (after migrations, in `db::init`) and rewrites each pre-0006 decimal cell of `trades`/`income` (exactly the columns that were REAL in 0004/0005) in `rust_decimal`'s canonical plain-decimal form via `decimal::canonicalize_decimal`. This expands the scientific-notation text SQLite's `CAST(REAL AS TEXT)` emits (e.g. `1.0e-08`) — which pure SQL cannot losslessly expand — without changing the numeric value; idempotent (already-canonical cells skipped, so a no-op on fresh DBs), runs in one transaction, and a malformed value fails loudly (decode error) rather than being silently rewritten. Tests: `decimal::tests::canonicalize_*` (expands small/large scientific notation, leaves plain decimals incl. preserved scale untouched, propagates malformed); `db::tests::canonicalize_pre0006_rewrites_scientific_notation_and_is_idempotent`, `canonicalize_pre0006_fails_loudly_on_malformed_value`
