...
- [ ] Add `sha2` dependency for the SHA-256 content checksum
- [ ] Attachment model — `src/entities/attachment.rs`, struct `Attachment`: id, exactly-one owner FK (`trade_id` / `income_id` / `amma_statement_id`, others null), `filename`, `content_type` (enum over the accepted MIME allowlist), `byte_size`, `checksum` (SHA-256 hex), `uploaded_at`. The `content` BLOB is loaded only by the download path, not on the metadata model
- [ ] DB schema: `attachments` table — three nullable owner FK columns, each `REFERENCES <activity>(id) ON DELETE CASCADE`; CHECK that exactly one owner is non-null; `content_type` CHECK enum (allowlist: `application/pdf`, `image/png`, `image/jpeg`); `content` BLOB NOT NULL; `byte_size` INTEGER, `checksum`/`filename`/`uploaded_at` TEXT. New migration. Note: requires `PRAGMA foreign_keys = ON` for the cascade — verify the pool sets it
- [ ] Upload endpoint: `POST /attachments` multipart/form-data (file + target owner) → 201 with metadata; computes `byte_size` + `checksum` server-side; enforces write-time invariants in a transaction — unsupported content type → 422, content type not in allowlist → 422, missing/>1 owner → 422, unknown owner activity (FK) → 422, oversized (> proposed 25 MB) → 413
- [ ] Metadata list/get: `GET /attachments` and `GET /attachments/:id` return metadata only (never the blob); list is filterable by owner (e.g. `?trade_id=` / `?income_id=` / `?amma_statement_id=`) so each activity's documents can be listed
- [ ] Content download: `GET /attachments/:id/content` streams the raw bytes with the stored `Content-Type` and a `Content-Disposition` filename; 404 if unknown
- [ ] Delete endpoint: `DELETE /attachments/:id` → 204, or 404 if unknown
- [ ] Cascade: deleting a Trade / Income / AMMA Statement removes its attachments atomically (`ON DELETE CASCADE`), so no orphaned blobs remain
- [ ] README sync: `attachments` table in the Database schema + Relationships sections; the attachment endpoints in the HTTP API section; 413 (and any new status) in Response codes
- [ ] Tests: upload returns 201 + metadata and stores content; checksum + byte_size computed correctly; round-trip download returns the exact bytes with the stored content-type + filename; content type outside the allowlist → 422; missing-owner and two-owners → 422; unknown owner activity → 422; oversized upload → 413; metadata list/get never include the blob; list filters by owner; deleting the owning activity cascades (attachments gone); delete unknown → 404
...
- [ ] Attachments UI on the Trade / Income / AMMA views — list an activity's attachments (filename, content type, size; via the shared `filterableTable`), upload a new file, download (link to `/attachments/:id/content`), and delete. Test asserts the view + the `/attachments` API paths it drives are present in the served `app.js` bundle (no browser harness)
