...
- [x] Add a shared reform module (`src/domain/cgt_reform.rs`) holding the commencement date (`1 July 2027`), the four gain categories and the reform predicates, each cited to EM Chapter 1 — one definition so no report can disagree about where the old law ends
...
- [x] Tests: a CGT event dated 2027-06-30 still computes under the discount unchanged; a post-commencement event inserted directly (`test_pool` + raw insert, since the write path rejects future dates) is refused by each report's guard, with the refusal naming the commencement date
- [x] NEEDS CLARIFICATION: pin the commencement date for the *app* against the Acts' own commencement table — Schedule 1 (except the minimum tax) and the Imposition Bill commence on the first 1 January/1 April/1 July/1 October after Royal Assent, and the amendments "largely apply in relation to assessments for the income year that includes 1 July 2027", while the ATO's page states 1 July 2027. Confirm whether any provision (the minimum tax's own commencement, the subitem-84 application rules) reaches a different date before hard-coding one
- [x] NEEDS CLARIFICATION: decide how the reform's worked examples are pinned by tests. Examples 1.1, 1.6–1.19 are dated 2027–2035, and the write path rejects any trade after today (`AmountsError::FutureDate`), so they cannot be entered "purely via the HTTP API" (`src/ato_examples.rs` convention) until those dates arrive. Choose between a report-level test that builds parcels/prices directly, a clock seam, or deferring the acceptance tests — and state the choice, because it is a departure from the existing convention
- [x] Docs sync: `docs/API.md` Known limitations + README/FEATURES state the reform's status (what is implemented, what is guarded), and `docs/ato/OVERVIEW.md`'s "How this maps" bullet gains the implementation sections here
- [x] Docs sync: an implementation section's `## ` heading is moved to `DONE/tax-domain.md` (its subject file) when closed, per [DONE.md](../DONE.md)'s table — the six sections are separate so they close and archive independently

Closed 2026-09-21. **The commencement date is pinned to 1 July 2027.** EM 1.214's
"first 1 January, 1 April, 1 July or 1 October after Royal Assent" is the machinery commencement
table, not the app's date; the app's date comes from the *application* rules and the ATO's own
statement — Division 119 applies to CGT events happening on or after 1 July 2027 (EM 1.216), every
other amendment applies in relation to assessments for the income year that includes 1 July 2027
(EM 1.219, i.e. 1 July 2027 – 30 June 2028), and QC 107304 states the changes "will apply from
1 July 2027". No provision reaches a different date for this app's single Australian-resident
individual, and the constant, its citation and the boundary are pinned by
`domain::cgt_reform`'s tests.

**The reform's worked examples are deferred, by decision.** `src/ato_examples.rs` enters its facts
purely through the HTTP API, and the shared `AmountsError::FutureDate` ceiling refuses any trade
dated after today — so Examples 1.1 and 1.6–1.19 cannot be keyed until their own dates arrive. A
clock seam was rejected because it would let the API accept a future-dated trade and so weaken the
write-time invariant the guard exists to back up. The choice is to pin them at the **report level**
(building parcels and prices directly) in the sections that implement the arithmetic each example
exercises, and it is stated in `docs/API.md`'s Known limitations and pinned by
`doc_checks::cgt_reform_commencement_guard_documented`.

**What landed.** `domain::cgt_reform` holds `COMMENCEMENT`, the four `GainCategory` variants in
statutory order, the `applies_to_event` / `discount_available` predicates, the guard functions and
`CgtReformError` (bridged into report code as a `sqlx::Error::Decode`, so it reaches the HTTP layer
as a logged `500`). `reports::realised_gains` refuses any Sell or rights sale dated on or after the
commencement date; `reports::net_capital_gain` refuses a post-commencement AMMA statement year end
and E10/G1/C2 event; `reports::parcel_optimiser`'s shared `disposal_figures` refuses a
post-commencement hypothetical disposal, which covers the pre-sale what-if too. Because
`reports::tax_report` and `reports::indexation_cross_check` read the realised rows through the same
guarded function, all four named reports refuse. Tests: the 2027-06-30 boundary still discounts; a
...
- [ ] NEEDS CLARIFICATION: whether to model the Minister's optional apportioning method (s 112-185) or default to market value and document the election as the taxpayer's own manual entry — the legislative instrument may not exist yet, and the EM leaves the method to it
  **Closed 2026-09-21 — decided: default to market value; the election stays the taxpayer's own manual entry.** Recorded as decision (a) in `docs/API.md`'s grouped *assets and situations outside this data model* limitation: the deemed disposal capitalises a held parcel at market value (the listing's stored 30 June 2027 `closing_prices` value), a missing value stays a `422`, and the s 112-185 alternative is not modelled because the method is left to a Ministerial legislative instrument that may not exist yet. Pinned by `doc_checks::cgt_reform_out_of_model_scope_decisions_documented` (commit `db4b773`).
- [ ] Pre-CGT assets: a trade dated before 20 September 1985 is **rejected at write time** today (`entities::trade::checks::CGT_START`, `AmountsError::PreCgtDate`; `docs/API.md` Known limitations), so a pre-1985 holding cannot be entered at all — and from 1 July 2027 there are no pre-CGT assets. Decide: admit the acquisition (so its pre-2027 gain is disregarded and only the post-2027 gain is taxed) or keep the refusal and document the gap. If admitted, `CGT_START` stops being a write-time floor and "pre-CGT gains are exempt" becomes date-bounded rather than a refusal
  **Closed 2026-09-21 — decided: keep the write-time refusal and document the gap.** Recorded as decision (b) in the same `docs/API.md` limitation: a trade dated before 20 September 1985 stays rejected (`entities::trade::checks::CGT_START`, `AmountsError::PreCgtDate`), so the reform's pre-CGT arms (EM 1.194–1.211, ss 112-175/112-180) are unreachable and a pre-1985 holding — including its 30 June 2027 reacquisition and any post-2027 gain — cannot be entered at all; admitting it would make `CGT_START` no longer a write-time floor and pre-CGT exemption a date-bounded rule. This section's conditional pre-CGT test arm is N/A on that decision. Pinned by the same `doc_checks` test.
...
**Closed 2026-09-21.** Every item above was recorded as a decision in `docs/API.md`'s grouped CGT-reform *assets and situations outside this data model* limitation — sub-items (c)–(i) — cross-linked to both ATO mirrors and the `docs/ato/OVERVIEW.md` mapping, with the user-facing surfacing in `docs/FEATURES.md` and `README.md`. Pinned by `doc_checks::cgt_reform_out_of_model_scope_decisions_documented`, which also extends `doc_checks::cgt_reform_mirrors_document_the_2027_changes` with the mirror paragraphs the decisions cite; `every_markdown_link_resolves` covers the new cross-links. Commit `db4b773`. One TODO cross-reference was stale: item 88's "existing *Property is not modelled* limitation" does not exist anywhere — the property scope cut is stated inside sub-item (c) and cross-referenced to *Collectables and personal-use assets* instead.
...
- [ ] NEEDS CLARIFICATION: decide how far Division 119 can be implemented, because steps 2–4 need a **basic income tax liability** on taxable income with and without the minimum tax capital gain — and this project computes no tax payable at all. There is no taxable-income total (`TaxYearSummary`'s `net_assessable_investment_income` is investment-only), no home for salary or other non-investment income (`income.listing_id` is `NOT NULL`, and `IncomeType` is Dividend/EmploymentIncome/OtherIncome with `EmploymentIncome` informational only), and no marginal-rate schedule anywhere in `src`. Options: (a) record the year's other taxable income in `tax_year_settings` and implement the resident rate schedule; (b) surface the *minimum tax capital gain* only and leave the gap to the taxpayer; (c) record the gap amount as a per-year user figure. (b)/(c) follow the FITO and ESS precedents; (a) makes the figure computable but adds a tax-payable engine this project has never had
  **Decided 2026-09-21 — option (c).** The app computes the minimum tax capital gain and the working it can derive, and the s 119-10(2) gap amount is the **taxpayer's own recorded figure** per year (`tax_year_settings.minimum_tax_gap_amount`, migration 0054), because steps 2–4 need a basic income tax liability on a taxable income this project has never computed. No marginal-rate schedule, no `basic_income_tax_liability`, no salary/other-income model — the FITO/ESS precedent. Recorded as clause (e) of `docs/API.md`'s reform Known-limitations entry and pinned by `doc_checks::division_119_minimum_tax_scope_decision_documented` (commit `9e13806`).
- [ ] If (a): extend `tax_year_settings` with the per-year taxpayer facts — the year's taxable income from sources outside the app, the income-support-payment exemption, and the residency answer the s 114-25 testing period needs — as a migration that DROPs and re-CREATEs the table's two `*_row_history_*` triggers with the new column list (it is an audited table; `migrations/0027_tax_year_settings.sql` is the pattern). Update `COLUMNS`/body in `src/entities/tax_year_settings.rs`, `docs/API.md`, `docs/SCHEMA.md` and the `tax_year_settings` SPA entry in `src/web/config.js`
  **N/A — option (a) was not chosen** (see the decision above). No non-investment taxable-income columns were added; the two columns migration `0054` did add are the recorded gap and the s 119-15 exemption, in `src/entities/tax_year_settings.rs`, `docs/API.md`, `docs/SCHEMA.md` and the SPA entry.
- [ ] If (a): the resident marginal-rate schedule and a `basic_income_tax_liability(taxable_income, tax_year)` function in a cited `domain` module (step 2's "basic income tax liability" under s 4-10(3), before offsets and before the Medicare levy). This is a new class of figure for the project — it needs its own tests against the ATO's published rates and a maintenance note: the schedule changes with each Budget, so a new rate year is a code change
  **N/A — option (a) was not chosen.** No rate schedule and no `basic_income_tax_liability` function exist; the gap is recorded.
- [x] Compute the **minimum tax capital gain**: the covered gains remaining after step 6 (s 119-5) — the post-1 July 2027, non-residential gains left after losses. Excluded: the deferred pre-2027 gains (EM 1.175), and new-residential/affordable-housing gains that chose the discount (N/A here)
  **Done**: `minimum_tax_capital_gain` is the seven-step walk's `net_non_residential_gain` for a reform year — the post-1 July 2027 non-residential gains left after the loss pool — and nil before it; the deferred pre-2027 gains are excluded (EM 1.175). Tests `db_a_deferred_gain_is_excluded_from_the_minimum_tax_capital_gain` and `db_a_pre_reform_year_has_no_minimum_tax`.
- [ ] Compute the seven-step **minimum tax gap amount** (s 119-10(2)): step 1 the 30 per cent benchmark; steps 2–4 the tax the gain already bears as the top slice; steps 5–6 the difference, rounded down to whole dollars; step 7 positive or nil. Then the extra rate is `minimum tax gap amount ÷ minimum tax capital gain` (Rates Act s 12AA)
  **N/A — the gap is recorded, not computed** (the decision above). The app derives the step-1 benchmark (30% × gain), the steps 2–4 already-borne figure (`benchmark − gap`, floored at nil) and the Rates Act s 12AA rate (`gap ÷ gain`) around the recorded figure; a negative gap is refused at write time with a `422`, and a gap above the benchmark is surfaced unchanged rather than clamped. Tests `db_em_example_1_17_genevieve_minimum_tax_working`, `db_the_s12aa_rate_reproduces_the_gap_for_a_non_terminating_rate`, `db_a_recorded_gap_above_the_benchmark_is_surfaced_unchanged`.
- [x] Apply the exemptions and exclusions: no minimum tax where the year carries a payment the Minister prescribes (intended to include the Age Pension, Disability Support Pension, JobSeeker, Parenting Payment, Youth Allowance, farm household allowance, ABSTUDY living allowance and the special rate disability pension — the instrument is not yet made), or where the taxpayer already pays at least 30 per cent on the gain. Record the exemption per year rather than inferring it (the ESS-eligibility pattern)
  **Done**: the s 119-15 exemption is recorded per year (`minimum_tax_income_support_exempt`, migration 0054), never inferred, and switches the whole Division 119 working off (`db_the_income_support_exemption_zeroes_the_minimum_tax_working`); the 'already taxed at 30 per cent or more' case is an absent/nil recorded gap (`db_no_recorded_gap_is_the_already_taxed_case`).
- [x] Decide where the figure surfaces: `reports::tax_summary` has no net-capital-gain field at all and its `total_assessable_income` (`tax_summary.rs:1127`) deliberately excludes capital gains, so there is no existing seam. Either add the reform fields to `TaxYearSummary` (and its CSV/label rows) or keep them on `reports::net_capital_gain` / `tax_report`'s `cgt_summary` alone — decide once, and say in `docs/API.md` where a reader should look
  **Done — decided**: the fields ride `reports::net_capital_gain`'s year row and `tax_report`'s `cgt_summary` (plus the SPA's net-capital-gain entry), never `TaxYearSummary`, whose `total_assessable_income` deliberately excludes capital gains; the reason is recorded in the report's module doc and in `docs/API.md`, with the seven fields in a new *Minimum tax (Division 119)* subsection and the CSV label table.
- [x] Surface the result as an informational figure with its working — the minimum tax capital gain, the 30 per cent benchmark, the tax on the gain as top slice, the gap and the effective extra rate — so a reader can check it and no liability the app does not otherwise compute is silently asserted. `reports::TAXPAYER_BASIS` must state the resident-individual and post-reform basis
  **Done**: the minimum tax capital gain, the 30 per cent benchmark, the recorded gap, the already-borne (top-slice) figure, the s 12AA effective rate and the extra income tax are all surfaced, with the uncomputed steps 2–4 stated as the taxpayer's own; `reports::TAXPAYER_BASIS` now states the resident-individual and post-reform basis.
- [x] Tests: EM Example 1.17 (Genevieve — a `\$50,000` minimum tax capital gain and `\$40,000` of other taxable income give step 1 `\$15,000`, step 4 `\$14,500`, a `\$500` gap and a 1 per cent extra rate = `\$500`). The EM's example states rounded tax figures against an illustrative schedule, so pin the test either to the rates its figures imply or to the enacted schedule for the year once legislated — and state which
  **Done**: `db_em_example_1_17_genevieve_minimum_tax_working` — a A$50,000 minimum tax capital gain with the A$500 gap the EM's figures imply gives the step-1 benchmark A$15,000, already-borne A$14,500, a 1 per cent extra rate and A$500 of extra tax. The test's doc comment cites EM 1.171–1.193 / Example 1.17 and states that steps 2–4 are the taxpayer's recorded figure, the EM's illustrative schedule being the EM's own rather than the app's.
- [x] Tests: no gap where the taxpayer's marginal rate already reaches 30 per cent; a prescribed income-support payment zeroes the gap; the rate formula reproduces the gap exactly
  **Done**: the no-gap case (an absent/nil recorded gap — the 'already at 30 per cent' taxpayer), the recorded exemption zeroing the gap, and the s 12AA rate reproducing the gap exactly for a non-terminating division.
- [x] Docs sync: `docs/API.md` (the new fields, the settings columns, and a Known-limitations entry for whatever Division 119 input remains the taxpayer's), README/FEATURES, and the `docs/ato/OVERVIEW.md` mapping
  **Done**: `docs/API.md` (the seven fields, the new Minimum-tax subsection, the CSV label row, the `tax_year_settings` columns and the negative-gap `422`), `docs/FEATURES.md`, `README.md`, the `docs/ato/OVERVIEW.md` mapping and the reform Known-limitations entry (clause (e)), with the `doc_checks` pins updated and `division_119_minimum_tax_scope_decision_documented` added.

**Closed 2026-09-21** — commits `f348fb2` (code, schema and tests) and `9e13806` (docs and pins). The `[ ]` items above are explicit decisions, not pending work: the gap is deliberately the taxpayer's own recorded figure (option (c)), which is what makes the option-(a) items N/A.
