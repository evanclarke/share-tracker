...
- [x] Decide whether to gate complexity in CI. `clippy::cognitive_complexity` is nursery-level and
  ...
  **Decision 2026-08-28: no CI gate. Not to be re-litigated without new evidence.** Measured again
  after the three refactors above: 15 distinct functions still exceed the default threshold of 25 —
  6 test assertion sequences (`row_history.rs:1623` at 93, five in `doc_checks.rs`) and 9 production
  functions (`scheduler/run.rs`'s `run_job` 45 and `run_entry` 38, `snapshot::run_snapshot_job` 34,
  `db::backup_to` 32, `http`'s `into_response` 30, `distribution_event::run_refresh` 30,
  `corporate_action::model::kind` 29, `period_performance::compute` 27, `health::db_health` 26). An
  allow-list of 15 is not "small", and every one of them was read during this audit and judged
  correct as it stands, so the gate would be recording the audit's exemptions rather than finding
  anything.

  Raising the threshold instead was considered, since the item's arithmetic assumed the default. At
  50 the allow-list is exactly one — the `row_history` test — and nothing in production is flagged.
  It was still not taken, for two reasons. The top production score is 45, so the margin is 10% on a
  **nursery** lint whose scoring is explicitly unstable: a clippy version bump that rescored
  `run_job` upward would fail CI under `-D warnings` on an unrelated PR (a Dependabot toolchain
  update, most likely), which is a poor trade for a ratchet that catches nothing today. And a
  threshold set high enough to need no production exemptions can only catch a function twice as
  complex as anything this audit judged acceptable.

  The deeper reason is that the audit's own findings argue against a single automated threshold.
  Neither tool ranks this codebase correctly on its own: the flat write-time guard chains score CCN
  57–129 and are correct as they stand, while `kind` — 129 on CCN — scores 29 on the cognitive scale
  and was the one worth rewriting; conversely the nesting outliers (`parse_iso4217`, nest 8) were
  invisible to *both* and needed a third measurement. The three items closed above came from reading
  the code with all three numbers in hand, which is a re-runnable audit, not a gate. The recipe is
  in this section's heading: `lizard` over `src` with `#[cfg(test)]` items classified out by
  brace-matching, cross-checked against `cargo clippy -W clippy::cognitive_complexity` and a
  per-function max-nesting-depth measurement. Re-run it if the tree grows substantially; the numbers
