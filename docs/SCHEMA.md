...
interest_income              Interest income — bank, term-deposit, or broker-cash interest (no listing, so not an income row). The tax summary reports the year's gross as its interest_income line (question 10 label L), includes it in gross assessable investment income, and joins the TFN amount withheld to the combined withholding line (label M)
├── id                    INTEGER PK
├── date_paid             DATE   Its month sets the ATO FX conversion month and the Australian financial year the interest is assessed in (a July date belongs to the next FY)
├── amount                TEXT (decimal)  Gross interest including any TFN amount withheld (the 10L convention)
├── tfn_withholding_tax   TEXT (decimal)  TFN amount withheld from the gross interest; joins the tax summary's withholding line
