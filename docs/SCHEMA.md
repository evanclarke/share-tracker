...
├── id           INTEGER PK       Autoincrement surrogate key (0039) — the row's identity for the audit trail (row_history.row_id); writes still address a holiday by (mic, holiday_date)
├── mic          TEXT FK→exchanges.mic   Part of UNIQUE(mic, holiday_date) — the former PK
├── holiday_date TEXT             'YYYY-MM-DD'; part of UNIQUE(mic, holiday_date)
...
├── unpriced_from TEXT (nullable)  'YYYY-MM-DD' — the date the **price provider** stopped quoting the security (0035): a delisting, or a suspension that can run for years (SCENARIOS Q-02). NULL = quoted as usual, which is what every existing listing means. From this date on, price collection stops fetching the listing (and /closing_prices/fetch + /backfill refuse or clamp at it), reports::health stops reporting its errored rows and unpriced days, and reports::valuation carries the listing's last stored ok close forward instead of blocking the whole portfolio's date — flagging the result price_carried_forward. Two pairings enforced in entities::listing::db_upsert (SQLite can neither ALTER in a table-level CHECK nor reference another table from a column CHECK): a stored ok price must exist **before** it (the figure carried forward), and no **fetched** ok price may exist on or after it (a *manual* one may — an administrator's valuation during a suspension, which valuation prefers to the carried-forward close)
├── unpriced_before TEXT (nullable)  'YYYY-MM-DD' — the date the **price provider's series begins** for the security (0037), before which nothing is obtainable at any price: the mirror image of `unpriced_from`, and the shape a spun-off entity has (New Lithium Americas' Yahoo series starts 2023-10-02; a backfill of any earlier range answers HTTP 400 on every day). NULL = its history runs as far back as the provider has it, which is what every existing listing means. Before this date, price collection never fetches the listing (and /closing_prices/fetch + /backfill refuse or clamp at it), reports::health stops reporting its errored rows and unpriced days, and reports::valuation **excludes the holding** from the date's portfolio totals instead of blocking them — flagging the snapshot holding_excluded and naming the absent holding in excluded_holdings. Deliberately **not** the mirror of the carry-forward: a close carried forward is a real, once-observed price, while nothing before the series begins was ever observed, so no figure is invented — the holding leaves the total and the total says so. The marker is a listing-level declaration and **supersedes** any closing price already stored for those days, whatever its origin (which is what lets a span priced from another security's series be retired) — retired one date at a time with `DELETE /closing_prices/:listing_id/:price_date`, which relaxes its ok-row refusal inside this span alone, or all at once with `POST /closing_prices/clear_unpriced_before`; every cleared row is kept in `row_history`). One pairing enforced in entities::listing::db_upsert (SQLite cannot ALTER in a table-level CHECK): it must fall strictly **before** `unpriced_from` when both are set — a security quoted only between two dates is exactly what the pair is for, but equal or crossed dates would leave no day the provider serves at all. There is deliberately **no** rule about prices already stored before the date, in either direction: nothing is carried so nothing need exist, and refusing one would make the feature unreachable for the case it exists for (the deployed database's LAC carries 635 such rows, 260 of them fetched under a one-off `symbol` override)
...
├── old_name         TEXT (nullable, 0040)  The listing's `name` at the moment of the rename, written from its own row like old_ticker. Restored by the undo, and doubles as the "this row recorded what it overwrote" marker: `listings.name` is NOT NULL, so a rename recorded from 0040 on always has one, and NULL means only that the row predates the columns (SCENARIOS R-04/R-08)
├── old_price_symbol TEXT (nullable, 0040)  The listing's `price_symbol` at the moment of the rename, likewise. With old_name present, NULL here is the real prior value ("the listing had no override") and the undo clears the one the rename set; with old_name NULL nothing was recorded and the undo leaves name/price_symbol alone — never invented from the current row, which would make an undo "restore" a listing to what it already is
└── note             TEXT (nullable)
                       CHECK: old_ticker <> new_ticker OR old_exchange_mic IS NOT new_exchange_mic (no no-op rename)
                       CHECK: old_name IS NOT NULL OR old_price_symbol IS NULL (an unrecorded row can carry no recorded symbol — what makes the NULL reading above enforceable)

rba_fx_rates                  RBA F11 monthly FX rates (the rate used for ATO conversion)
├── id           INTEGER PK       AUTOINCREMENT — never reused, so an audit trail belongs to one row
├── currency     TEXT             ISO 4217 code (e.g. USD)
├── month        TEXT             'YYYY-MM'
└── rate         TEXT (decimal)   Foreign units per 1 AUD; UNIQUE (currency, month).
                                  Created only by the import (ON CONFLICT DO NOTHING — a scheduled
                                  run never rewrites a figure a lodged return used); corrected in
                                  place by PUT /rba_fx_rates/:id, which is why the table is audited
                                  (0031) and why a rate change stales every snapshot from its month
...
├── settlement_date_source TEXT (0041)  computed | stated | unrecorded — where the stored settlement_date came from, CHECK-constrained. computed: derived from the exchange calendar because the write omitted it, and so the only kind the settlement-recompute job (POST /jobs/settlement-recompute) re-derives once a missing holiday year is seeded. stated: asserted by its writer — supplied in a PUT /trades/:id or PUT /sells/:id body, or written same-day by a derived path (ESS vest, inherited parcel, DRP reinvestment, rights exercise, rollover trade) — and never rewritten. unrecorded: the row predates this column (the ADD COLUMN default, so every pre-0041 row), provenance unknowable, also never rewritten. Re-supplying the date already stored keeps the recorded value: handing back a GET body is not an assertion
├── listing_id        INTEGER FK→listings.id
...
├── fx_rate           TEXT (decimal)  Manual foreign-per-AUD override; fallback when no ATO rate exists (1.0 for AUD trades). On an ESS vest Buy it is the statement's stated `ess_statements.fx_rate`, or — absent one — the taxing-point month's ATO rate resolved when the vest ran (never a placeholder 1: a foreign vest with neither is refused)
...
├── deemed_acquisition_date TEXT (nullable)  Scrip-for-scrip replacement, demerger head/demerged, transfer-in and inherited-parcel Buys only: the consumed parcel's (or the deceased's) acquisition date, carried by the rollover/transfer (the combined holding period; an own-account transfer is not a disposal at all). Drives the 12-month CGT discount clock and the AUD translation month of the cost base in the reports; split/return-of-capital applicability stays on the actual trade date — with one exception on a **rollover replacement** parcel (`scrip_action_id`/`demerger_action_id`/`transfer_id` set, which is how the reports tell it from an inherited parcel): the operation folded every return-of-capital payment up to **and including** its own date into the cost base the parcel carries, so a payment dated on the trade date is already inside that figure and is not applied a second time (`domain::cost_base`'s `rolled_over_on`)
...
tax_year_settings            Per-financial-year taxpayer settings (0027) — facts about the taxpayer, not a holding, answered year by year. An absent row means every setting takes its default, so an empty table behaves exactly as the system did before the table existed
├── tax_year                             INTEGER PK  The Australian financial year, by the calendar year of its 30 June end (domain::tax_year). CHECK: >= 1986 (CGT starts 20 September 1985, inside FY1986). Also the audit trail's row_id — a meaningful integer never reused for another fact, so a deleted-and-re-entered year inherits its own history
...
├── demerger_close_date  TEXT (date, nullable, 0036)  Demerger only: the last PRE-demerger trading day (CHECK: only on Demerger rows, and strictly before `date`). One half of the demerger's price re-basing factor — a PRICE fact only, read by nothing that computes a quantity or a cost base. NULL = not stated, and the listing's pre-demerger closing prices stay as the provider served them (reported by GET /reports/health's demergers_missing_close)
├── demerger_close_price TEXT (decimal, nullable, 0036)  Demerger only: what the security ACTUALLY closed at on demerger_close_date, in the listing's quote currency (positive). The other half of the factor: the price provider restates a security's whole pre-demerger series by its spin-off adjustment factor exactly as it does for a split, but a demerger changes no unit count on the head listing, so there is no ratio to read (demerger_cost_base_pct is an ATO cost-base apportionment, not a price ratio). closing_prices.price is re-derived over this stated close divided by the provider's own stored figure for the same day — both sides kept as facts, the quotient computed at re-base time, so the close can be stated before the history is backfilled. Present exactly when demerger_close_date is (CHECK)
├── demerger_close_sourced_from TEXT (nullable, 0036)  Demerger only: where the stated close was taken from — the provenance a hand-entered closing price carries (0020), for the same reason: it is a figure a person asserted. Informational; not read by the arithmetic. Present exactly when demerger_close_date is (CHECK)
├── demerger_close_reason TEXT (nullable, 0036)  Demerger only: why the close had to be stated. Informational, as demerger_close_sourced_from. Present exactly when demerger_close_date is (CHECK)
...
├── price       TEXT (decimal, nullable)  Closing price in the listing's quote currency (never AUD-converted; reports convert at read time), **in the unit basis in force on price_date** — the price the security actually traded at that day. The one exception is a **demerger's own date**: the re-base walk covers only the rows dated strictly before the event, so that day's row keeps the provider's spin-off-adjusted figure — its standalone-equivalent estimate for the head listing, which is what is wanted there because the demerged parcel is already held (and valued) on that date. See API.md Closing prices. NULL exactly when status = error (CHECK)
├── price_as_observed TEXT (decimal, nullable, 0034)  The figure exactly as the provider served it (or as the operator entered it), in the unit basis in force when it was observed — which fetched_at dates. `price` is derived from it by the price re-basing events dated in (price_date, fetched_at] — the ShareSplit/BonusIssue ratios, plus a factor derived from each Demerger's stated pre-demerger close (0036) — so recording, editing or deleting one re-derives the price from source rather than adjusting an already-adjusted number (idempotent, order-free). That event set is a strict SUPERSET of the quantity re-basing one: a demerger restates the price series but moves no unit count, so it never enters the split ratio the cost-base and allocation-capacity walks read. Equal to `price` for a manual row. NULL exactly when status = error (CHECK), so no ok row can exist without the observation a re-base must re-derive it from
...
├── fetched_at  TEXT             RFC 3339 UTC timestamp of the fetch — for a manual row, of the entry that recorded it. Also **dates the unit basis** price_as_observed arrived in: the provider restates its whole close series into the current basis when a security splits, so a figure observed before a split (or demerger) is already contemporaneous and one observed after it is not (0034/0036, SCENARIOS Q-14 and the demerger finding that followed it)
├── fetched_symbol TEXT (nullable, 0038)  The provider symbol this row was actually fetched under, in `source`'s namespace — recorded on **every** fetched row, ok or errored, so a backfill made with the one-off `symbol` override is afterwards distinguishable from an ordinary fetch (the incident it exists for stored 260 rows of another security's series and recorded nothing). Always recorded, never only-when-it-differs: that would make NULL mean both "ordinary fetch" and "not recorded". NULL when origin = manual (CHECK — nothing was fetched), for rows stored before 0038 (unrecorded, and not recoverable after the fact — no migration invents one), and for the errored row a fetch stores when no symbol could be resolved at all (an exchange with no provider mapping, which the row's `error` names). Informational: no calculation reads it
...
├── price_carried_forward INTEGER  0 | 1 (CHECK, 0035): 1 = a held listing was valued at a **carried-forward** close because the provider has stopped quoting it (listings.unpriced_from). Deliberately a separate flag from provisional: that one is an interim FX rate a later import trues up, and the true-up runs target provisional dates — a carried-forward price never clears, so folding the two together would turn a bounded true-up into one that never finishes. Nothing regenerates it; clearing unpriced_from stales those dates instead (the listings staleness trigger, below)
├── holding_excluded INTEGER      0 | 1 (CHECK, 0037): 1 = the stored totals **omit** a held holding, because no price is obtainable for it at this date (listings.unpriced_before). Its own flag, not folded into either of the two above: `provisional` is trued up by the runs that select on it and an excluded holding never clears (folding it in would make that pass unbounded), and `price_carried_forward` is the weaker statement — that one says the figure rests on an interim input, this one says the figure is missing a holding. Nothing regenerates it; clearing unpriced_before stales those dates instead (the listings staleness trigger, below)
├── excluded_holdings TEXT         JSON array of `{"listing_id", "ticker", "reason"}` — which holdings the totals omit and why, exactly as the generation run resolved it (0037). `'[]'` when none, which is what every existing row means. Stored rather than derived, so the answer stays readable after the listing's marker moves; the boolean says the total is incomplete, this says which holding is absent
...
job_runs                     Run history of the scheduled/on-demand maintenance jobs (one row per run, inserted when the run starts and updated when it finishes; pruned to the newest 20 per job in the same write as the insert)
...
├── finished_at TEXT (nullable)  RFC 3339 timestamp the run ended; NULL while status = 'running' (0042)
├── status      TEXT             running | ok | failed (CHECK, 0042). 'running' from the moment the run starts until it returns — so a row left 'running' is a run that started and never finished, which is what an interrupted process leaves behind. Replaced 0012's success boolean: a started-and-unfinished run is neither a success nor a failure, and a second CHECK holds it in step with finished_at ('running' exactly while there is no finish time)
└── error       TEXT (nullable)  Human-readable error when status = 'failed', else NULL
...
exchanges ──< listings ──< trades >──────────────< parcel_allocations
                       ...
                       listings (currency/security_type only, 0030) ──> report_snapshots.stale
                       rba_fx_rates (a corrected rate, 0031) ──> report_snapshots.stale
                       exchange_holidays (the trading calendar valuation reads live, 0033) ──> report_snapshots.stale
                       exchange_holidays (audited from 0039; the surrogate id row_history keys on comes from the same rebuild)
...
`report_snapshots` has no foreign keys of its own (`report` is an in-code enum, the rows are a JSON payload), but every dated fact table writes to it through **staleness triggers** (0001_schema.sql): inserting, updating, or deleting a row in `trades`, `parcel_allocations` (dated by its sale trade), `income` (by `date_paid`), `amma_statements` / `amit_adjustments` (by the statement's `tax_year_end_date`), or `corporate_actions` sets `stale = 1` on every snapshot dated on or after the fact — an update from the earlier of the old and new dates — atomically with the fact write, so no write path (entity CRUD, Sells, transfers, corporate-action operations, DRP reinvestment) can bypass the invalidation. Revising a stored ok closing price (or erroring it out) likewise stales snapshots from its date, since they were valued at it. `closing_prices` carries only that one `AFTER UPDATE` trigger — no INSERT or DELETE counterpart — because neither can invalidate a snapshot: a new price fills a date that was blocked (so no snapshot exists to stale), and the only deletable rows are ones no stored figure was valued at — an errored row, whose date valuation blocks outright, and (since 0037) a row inside the listing's `unpriced_before` span, where the marker supersedes the stored rows and valuation excludes the holding from the date's totals rather than pricing it. `DELETE /closing_prices/:listing_id/:price_date` rejects an ok row 422 anywhere else, and `POST /closing_prices/clear_unpriced_before` — the bulk clear of a superseded span — is bounded by the listing's own marker, never a caller's date range. Setting or moving that marker is itself what stales the affected snapshots (see `listings` below), so clearing the rows it supersedes moves no stored figure. A **manually entered** price (0020) changes neither premise: it is stored `status = ok`, so it is undeletable like any other price, and replacing a stored ok price with one is an ordinary UPDATE the trigger already catches. Re-basing a stored price when a `ShareSplit`/`BonusIssue` is recorded, edited or deleted (0034) is likewise an ordinary UPDATE, so the valuations that used the superseded figure are staled and regenerate. `rba_fx_rates` carries an `AFTER UPDATE` trigger (0031) firing only when the `rate` itself changes: an import filling a month that had no rate is the provisional true-up's business (a provisional snapshot is *regenerated*, not staled — the facts were never wrong, only the rate was interim), but changing a stored rate means every snapshot from that month on was valued at the old figure, so the suffix from its first day is staled. `listings` carries its own single `AFTER UPDATE` trigger (0030, re-created by 0035 and again by 0037), narrowed by a `WHEN` clause to `currency`, `security_type`, `unpriced_from` and `unpriced_before` — the columns that change what a *stored* snapshot's figures mean: the currency denominates every stored closing price (so a change re-values the whole price history), the security type decides which days a listing can be valued on (a Crypto listing trades every calendar day, everything else only on its exchange's), and the two dated markers bound the span the price provider serves it, deciding whether the listing is valued at the day's own close, at a **carried-forward** one, or not at all. The first two carry no date of their own, so they stale **every** snapshot rather than a suffix; `unpriced_from` does carry one, so it stales the **suffix** from the earlier of its old and new dates — which is what makes *clearing* it (the security relists, the suspension lifts) put the real prices back into the stored figures instead of leaving a flat line at the last close. `unpriced_before` is its mirror in both respects: it stales the **prefix**, before the *later* of its old and new dates, so clearing it (or moving it back) puts an excluded holding back into the stored totals. Both bodies live in the one trigger rather than a parallel mechanism, since `reports::snapshot`'s classification test pins the exact trigger set `listings` carries. An edit to `name`, `isin` or `price_symbol` fires nothing, since a series left permanently stale by a name change would teach a reader to ignore the flag. It needs no INSERT or DELETE counterpart: a listing with no trades is held on no snapshot date, and a delete is refused while any row references it. `exchange_holidays` carries a full insert/update/delete set (0033), keyed on `holiday_date`. It is reference data rather than a dated fact, which is why it was missed, but it is an input to the **valuation itself**, not merely to a value persisted at write time: every snapshot values each holding at its nearest trading day on or before the snapshot date, and that walk reads this table live on every generation. Adding a holiday moves the valuation back to the prior close (observed: a 12.4% move on a stored figure nothing had flagged), and deleting one makes the date a trading day whose price was never collected, leaving the stored figure resting on a valuation day that no longer exists. A holiday only affects snapshots dated on or after it — an earlier one values at an earlier trading day and can never reach it — so all three arms stale the suffix from its date (the update arm from the earlier of the old and new dates), without narrowing to the listings trading on that `mic`, which is a per-date question a trigger cannot answer cheaply. The update arm is `WHEN`-narrowed to the date/`mic` actually changing: the only column an UPDATE can reach through the API is the informational `name`, and staling years of snapshots for a "Queen's Birthday" → "King's Birthday" correction would teach a reader to ignore the flag. `rights_sales` / `rights_sale_allocations` and `interest_income` are the deliberate exceptions: no snapshotted report reads them (a rights sale changes no holding quantity and no parcel cost base — its effect is confined to the live-computed CGT reports; interest reaches only the tax summary, which is not snapshotted), so they carry no trigger set. Which tables carry a set — and which are exempt, with the reason — is pinned against the live schema by `reports::snapshot`'s `every_table_is_classified_for_snapshot_staleness` test (the sibling of the audited-table pinning below): every table here is in one list or the other, and a table in neither fails, so a new dated fact table cannot land without triggers or a stated exemption.

`row_history` is the **append-only audit trail** (0013_row_history.sql). `AFTER UPDATE` / `AFTER DELETE` triggers on every audited table record the prior row (as JSON), the operation, and a UTC timestamp — in the writing transaction itself, so no write path can bypass it and a rejected (rolled-back) write leaves no phantom entry, while cascade deletes (an activity's attachments, a rights sale's allocations) are recorded like direct ones. `table_name`/`row_id` are deliberately *not* foreign keys: a history entry must outlive its row (the DELETE case). Audited (scope decision 2026-07-14): every user-entered table whose values feed a calculation — the financial fact tables plus `cgt_settings` and, of the reference data, `listings` alone (its `amit`/`amit_from`/`security_type`/`preference` flags retroactively change tax calculations, and `unpriced_from`/`unpriced_before` decide whether a stored valuation is the day's own close, a carried-forward one, or absent from the total altogether). `listing_renames` (0018) joined the audited set too, which required rebuilding `row_history` itself to extend its `table_name` CHECK (a table-level CHECK SQLite cannot `ALTER`) — the *live* `row_history_append_only_*` guard triggers, and the `listings` trigger pair carrying the new `price_symbol` column, come from 0018, not 0013. `listing_renames`' own pair was re-created in 0040, which added `old_name`/`old_price_symbol` — what a rename overwrote, so undoing one puts back the `name` and `price_symbol` it changed as well as the ticker (SCENARIOS R-04/R-08); an `ALTER TABLE ADD COLUMN` like 0038's, with the trigger pair still dropped and re-created. The listings pair was re-created once more in 0024, which added `amit_from`, again in 0035, which added `unpriced_from`, and again in 0037, which added `unpriced_before`. `closing_prices` joined the audited set in 0021: 0020 made a price **hand-enterable**, which retired the "import-managed and re-importable" ground its original exclusion rested on — a manual price is a user-entered value feeding every valuation, and overwriting one with another used to discard the superseded figure and the `sourced_from`/`reason` given for it. Auditing it needed a row identity `row_history.row_id` could key on, so 0021 rebuilt the table with an AUTOINCREMENT surrogate `id` (the old composite key kept as `UNIQUE(listing_id, price_date)`) and rebuilt `row_history` again to extend the `table_name` CHECK. Because `closing_price::db_store` upserts on the natural key, a replacing write is an UPDATE and lands in the trail; the DELETE trigger covers discarding an errored row, the only delete the API allows. Its trigger pair was re-created once more in 0034, which rebuilt the table a third time to add `price_as_observed` — the figure each price was observed as, which is what lets a split recorded later restate the stored price from source instead of from itself (SCENARIOS Q-14). A re-base therefore lands in the trail like any other price revision, carrying the observation with it. It was re-created a second time in 0038, which added `fetched_symbol` — the provider symbol each fetched row was fetched under, the provenance the one-off `symbol` override recorded nowhere; that one is a plain `ALTER TABLE ADD COLUMN` (its CHECK is a column constraint, so no fourth rebuild was needed), but the trigger pair still had to be dropped and re-created, since a column the trail drops is a version of the row that can never be recovered. `tax_year_settings` joined in 0027 for the same reason `cgt_settings` is audited — a taxpayer-level fact that changes an assessable total — and again required rebuilding `row_history` to extend the `table_name` CHECK, so the *live* append-only guards now come from 0027. It needed **no** surrogate key: `tax_year` is already an integer identity and is what `row_id` records, and unlike a (listing, date) price row it is never reused for a different fact, so a deleted-and-re-entered year inheriting its own trail is right rather than a leak. `rba_fx_rates` joined in 0031, the `closing_prices` story exactly: its exclusion also rested on being "import-managed and re-importable", and that held only while *nothing* could change a stored rate — which was itself the defect, since the import is `ON CONFLICT DO NOTHING` and a rate that landed wrong (a typo in a hand-pasted retry CSV, a truncated download, an upstream revision) could be fixed only by editing the database by hand. `PUT /rba_fx_rates/:id` corrects one, so the superseded figure — which every tax figure in that currency-month rested on — lands in the trail. Its `id` was already `AUTOINCREMENT` (0001), so no rebuild of the table was needed; `row_history` itself was rebuilt again to extend the `table_name` CHECK, so the *live* append-only guards now come from 0031. `exchange_holidays` joined in 0039 (decided 2026-08-21), the last of the falsified exclusions. It and `exchanges` were originally excluded together as tables that "only influence values persisted onto trades at write time" — true of a settlement date, which is computed once from the calendar and then stored on the trade. That ground was wrong for the calendar itself (corrected 2026-08-20, SCENARIOS Q-05/Q-08): `reports::valuation::stored_valuations` reads it **live** on every snapshot generation, walking each holding back to the nearest trading day on or before the date, which is why 0033 had to give it staleness triggers at all. So a hand-edited holiday changes a reported figure — the audited set's own stated criterion — and it has been hand-editable by `PUT`/`DELETE` from the start, with no import to re-derive it from (the seed is a one-off in 0001_schema.sql), leaving a deleted holiday unrecoverable from anything else the database holds. The staleness triggers flag the *effect* of a change; the trail is what retains *what* was changed. Auditing it needed a row identity `row_history.row_id` could key on, the natural key being composite, so 0039 rebuilt the table with an AUTOINCREMENT surrogate `id` (the old primary key kept as `UNIQUE(mic, holiday_date)`, so `db_upsert`'s `ON CONFLICT` target still resolves and every route stays keyed on the natural key), re-created 0033's three staleness triggers with it, and rebuilt `row_history` once more to extend the `table_name` CHECK — so the *live* append-only guards now come from 0039. Both audit triggers record `name` too: informational to the calculations, but a trail that dropped it could not say which holiday a deleted date was. Neither is `WHEN`-narrowed the way the staleness trigger is — staleness asks whether a stored figure changed, to which a name correction answers no, while the trail asks what the row said before the write. **`exchanges` stays out, and its half of the original sentence remains true**: `settlement_days` is consumed when a trade is written and persisted onto the trade, and `timezone`/`close_time` decide only which dates are *generable*, never what a stored snapshot says. The remaining import-managed reference data (`currencies`, `mic_registry`), `exchanges`, the identity-only `holding_accounts`, and derived state (`report_snapshots`, `job_runs`) are out of scope. Retention (decision 2026-07-14): entries are kept forever — the table is itself append-only, enforced by its own `BEFORE UPDATE`/`BEFORE DELETE` `RAISE(ABORT)` triggers, and deliberately has no pruning job. A migration adding a column to an audited table must recreate that table's two `*_row_history_*` triggers with the new column list.

`rba_fx_rates` is standalone reference data (no foreign keys); it is looked up by `(currency, month)`. `job_runs` is likewise standalone: `name` is the in-code job name (not a foreign key), and each scheduled or manual run appends a row **when it starts** (`status = 'running'`, no `finished_at`), updating that same row when the work returns; the insert prunes that job's history to the newest 20 rows in the same transaction — so an intermittent failure that later succeeds stays visible (surfaced by `GET /jobs` and the [health report](API.md#health)) without the table growing unboundedly. Recording the start is what makes an *interrupted* run visible (SCENARIOS T-11, migration 0042): the row used to be written only after the work returned, so a restart landing on the weekly backup's own slot left no trace at all and `GET /jobs` went on showing the previous run's result. The prune can never take the row an in-flight run is about to update — that row is the newest of its job, and the per-job lock keeps any other run of it from inserting meanwhile. `cgt_settings` is also standalone: a singleton row (`CHECK (id = 1)`) holding the entered opening carried-forward capital loss consumed by the [net capital gain report](API.md#net-capital-gain). `tax_year_settings` (0027) is standalone too, and is the *per-year* counterpart of that singleton: its key is the financial year itself rather than a foreign key, and the [tax summary](API.md#tax-summary) reads it to decide whether a year's $1,000 taxed-upfront ESS reduction applies. Per year rather than singleton because the income test behind it is answered year by year and the summary reports every recorded year at once — one global flag would strip the reduction from years that never crossed the threshold.
