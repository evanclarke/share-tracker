...
├── price       TEXT (decimal, nullable)  Closing price in the listing's quote currency (never AUD-converted; reports convert at read time), **in the unit basis in force on price_date** — the price the security actually traded at that day. NULL exactly when status = error (CHECK)
