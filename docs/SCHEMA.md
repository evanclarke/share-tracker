...
├── price_as_observed TEXT (decimal, nullable, 0034)  The figure exactly as the provider served it (or as the operator entered it), in the unit basis in force when it was observed — which fetched_at dates. `price` is derived from it by the ShareSplit/BonusIssue actions dated in (price_date, fetched_at], so recording, editing or deleting one re-derives the price from source rather than adjusting an already-adjusted number (idempotent, order-free). Equal to `price` for a manual row. NULL exactly when status = error (CHECK), so no ok row can exist without the observation a re-base must re-derive it from
...
├── fetched_at  TEXT             RFC 3339 UTC timestamp of the fetch — for a manual row, of the entry that recorded it. Also **dates the unit basis** price_as_observed arrived in: the provider restates its whole close series into the current basis when a security splits, so a figure observed before a split is already contemporaneous and one observed after it is not (0034, SCENARIOS Q-14)
