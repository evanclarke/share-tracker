...
- a **share split** falls between the covered parcels' acquisition dates and the year end, leaving them on different unit bases — one per-unit `cost_base_adjustment` cannot correctly scale both sides of a split. (A pre-existing modelling limit that hand entry has too, silently; a split before *every* covered parcel scales them all alike and generates normally.)
...
- `ReturnOfCapital` returns `422` while the listing has **any parcel acquired on or before** the payment `date` — a parcel the payment reduced. Deleting it restores the cost base it reduced and drops any CGT event G1 excess gain already reported for the payment's income year. Delete those parcels first.
...
- **Editing a split, bonus issue, or return of capital in place restates prior figures** (2026-08-14) — deleting one of these three [corporate actions](#corporate-actions) is guarded once trades depend on it (see *Deleting an action that is already depended on*), but `PUT /corporate_actions/:id` is deliberately **not**: a mis-keyed ratio, date, or per-unit amount stays correctable in place, because the alternative — freezing the action the moment anything depends on it — would mean deleting every later trade of the listing to fix a typo. The consequence is that an edit recomputes every open-parcel quantity, cost base, and realised gain that reads the action, including figures in an already-lodged year, with nothing marking the restatement (the change itself is recorded in [row history](#row-history)). There is no lodged/closed-year concept in the data model; check the affected years' reports after editing one of these actions.
