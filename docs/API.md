...
| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/attachments` | List attachment metadata (never the blob); filter by owner with `?trade_id=`, `?income_id=`, `?amma_statement_id=`, `?ess_statement_id=`, or `?interest_income_id=` |
