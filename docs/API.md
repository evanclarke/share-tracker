...
- **A manually entered price is one-way and is not itself audited** (2026-07-28) — a hand-entered [closing price](#closing-prices) can only ever be replaced by another hand-entered one: the scheduled job and backfill skip it as an already-ok day, an explicit re-fetch is refused `422`, and it cannot be deleted (an ok row never is). So a day priced by hand stays that way even if the provider later starts serving it correctly, and a manual price entered against the wrong date cannot be removed — only overwritten with the right figure. That one-way rule is what keeps a valued price from being deleted out from under a stored snapshot. Separately, `closing_prices` is outside the [row-history audit trail](#row-history) (it has no integer `id` for `row_history.row_id` to key on), so overwriting one manual price with another keeps only the latest `sourced_from`/`reason` — the superseded provenance is not retained.
