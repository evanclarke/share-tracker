...
Fields: `date_paid` (its month sets the financial year and the ATO FX conversion month), `amount` (the **gross** interest including any TFN amount withheld — the return's 10L convention), `tfn_withholding_tax` (the withheld amount, 10M), `currency` (defaults to AUD), an optional free-text `source` description (e.g. the bank account — informational only), and an optional `holding_account_id` link for interest paid on a portfolio account such as a broker cash account (NULL for an ordinary bank account; informational only). `PUT` returns `422` for an unrecognised `currency` or `holding_account_id`, or a negative `amount` or `tfn_withholding_tax` (interest figures are the statement's positive — or zero — amounts).
...
**Interest income** (`docs/ato/tax-return-labels-2026.md`, question 10): [interest income](#interest-income) records are aggregated by `date_paid` (July = next FY) into `interest_income` — the year's gross interest in AUD (foreign-currency amounts converted via the ATO rate for the month paid; no rate ⇒ fails loudly with `500`). The gross figure includes any TFN amount withheld (the 10L convention); the withheld amount itself joins the existing `tfn_withholding_tax` line. Every interest record reports at 10L regardless of source — foreign broker-cash interest strictly belongs at 20E instead (see [Known limitations](#known-limitations)).
...
- **Pre-CGT holdings** (2026-06-10) — a parcel acquired before **20 September 1985** is outside CGT entirely (gains and losses on it are disregarded). There is no pre-CGT flag, so the system would wrongly compute a capital gain or loss on such a parcel — pre-CGT holdings are not modelled and should not be entered. (The one modelled pre-CGT interaction is an inherited parcel that was pre-CGT *in the deceased's hands*: [Inheritances](#inheritances) applies the market-value-at-death cost base, and the parcel is post-CGT in the beneficiary's hands.)
...
- **Foreign broker-cash interest classification** (2026-06-12) — the [tax summary](#tax-summary) reports **all [interest income](#interest-income) at question 10** (gross interest, label **10L**) regardless of source. Strictly, interest-like income on **foreign broker cash / money-market sweep funds** (e.g. a US broker's Treasury liquidity fund distributions) is assessable foreign source income belonging at label **20E** (`docs/ato/tax-return-labels-2026.md`), with any foreign tax withheld claimed via the question 20 FITO. The single-question simplification is deliberate — the report has one interest line and no per-record source split; a taxpayer with material foreign interest reclassifies the 10L figure to 20E when lodging.
