...
- `PUT /trades/:id` and `DELETE /trades/:id` return `422` if the existing trade belongs to a holding-account transfer group (`transfer_id` set) — the group only ever changes as a whole, via `DELETE /transfers/:id` (see [Transfers](#transfers)) — or it is an [ESS vest Buy](#ess-statements) (`ess_statement_id` set): edit by deleting the [ESS statement](#ess-statements) and re-vesting.
