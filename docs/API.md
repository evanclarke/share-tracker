...
- **`income`** — per-record detail (not just year totals) across: `trust_income` (non-AMIT trust income rows) and `amma_statements` (the full AMMA component breakdown for the year); `dividends` (non-trust income rows, each carrying its franking entitlement `franking_status` — `entitled`, `denied`, or `exempt_small_shareholder` — from the [franking at-risk](#franking-at-risk) walk, so a denied credit is visible on the row, not just missing from a year total); `foreign_income` (every foreign-source amount — dividend/trust, AMMA, interest, the ESS foreign-source memo — with its foreign tax paid); `interest`; `ess`; and `deductions`. Every AUD figure here sums to exactly the matching [tax summary](#tax-summary) line for the year.
