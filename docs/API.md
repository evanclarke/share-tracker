...
The provider behind the pluggable fetcher is **Yahoo Finance** (unofficial chart API, via the `yfinance-rs` crate): free and keyless, covering ASX (`.AX` suffix), NYSE/Nasdaq (plain ticker), and crypto (`<TICKER>-<currency>`) in one source. A **failed fetch is stored as an errored row** (`status: "error"`, `price: null`, `error` text) — never silently missing — and is replaced by a later successful re-fetch. Yahoo serves float32-precision values, so prices are rounded to 7 significant digits before storing. Prices for other exchanges need a symbol mapping added to the fetcher first, or a **[`price_symbol` override](#listings)** set on the listing (used verbatim, ahead of the derived mapping) — until either is done their fetches store errored rows naming the exchange. A provider call that returns **zero candles across the whole requested window** (as opposed to a data gap on one date among others) stores that distinct message on every date in the window, naming the symbol and pointing at the fix (`price_symbol`, or an explicit `symbol` on backfill) — the classic wrong/renamed/delisted-symbol case, which would otherwise read identically to a transient outage. `GET /reports/health`'s `errored_prices` list (see [Health](#health)) surfaces every listing with any errored row, since a stuck symbol otherwise only shows up indirectly as a missing snapshot.
...
- **`income`** — per-record detail (not just year totals) across: `trust_income` (non-AMIT trust income rows) and `amma_statements` (the full AMMA component breakdown for the year); `dividends` (non-trust income rows, each carrying its franking entitlement `franking_status` — `entitled`, `denied`, or `exempt_small_shareholder` — from the [franking at-risk](#franking-at-risk) walk, so a denied credit is visible on the row, not just missing from a year total); `foreign_income` (every foreign-source amount — dividend/trust, AMMA, interest, the ESS foreign-source memo — with its foreign tax paid); `interest`; `ess`; and `deductions`. Every AUD figure here sums to exactly the matching [tax summary](#tax-summary) line for the year. Each row's `ticker` resolves **as at its own date** (pay date, AMMA year end, or taxing point) across any [rename](#listings), for the same reason as the disposal schedule.
...
Print path: the web UI renders this as a plain semantic document (not the shared `filterableTable` — a print document has no business with a filter row, sort indicators, or a 50-row pager, which would silently print only the first page) with a Print / Save as PDF button (`window.print()`) over a dedicated `@media print` stylesheet.
...
Operational health / data-freshness in one read (a single read transaction), driving the web UI's cross-view banner: a failing import or silently aging price/FX data is surfaced on every screen rather than only when the Jobs page is opened. Returns `{ "latest_price_date", "prices_stale", "latest_fx_month", "fx_stale", "failed_jobs", "errored_prices" }`:
...
- `errored_prices` — every listing with at least one errored [closing price](#closing-prices) row, newest error first: `{ "listing_id", "ticker", "errored_days", "latest_errored_date", "latest_error" }`. A wrong, renamed, or delisted provider symbol otherwise only shows up indirectly, as a missing snapshot from the errored date onward (`stored_valuations` refuses to value a date with an errored price) — this is the surface that names it directly. Fix the symbol (`price_symbol` on the listing, or record the rename via [`POST /listings/:id/rename`](#listings)) then re-fetch via `/closing_prices/backfill` or `/fetch`.
