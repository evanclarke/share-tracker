...
`POST /jobs/:name` runs the job synchronously and returns `204 No Content` on success, `404 Not Found` if no job has that name, or `500 Internal Server Error` if the job fails. Either way the run is recorded (see `GET /jobs`). Registered jobs are `backup`, `rba-fx-import`, `mic-import`, `currency-import`, and `price-import` (see [Closing prices](#closing-prices); scheduled twice daily — after the ASX close and after the NYSE close / crypto UTC-midnight cut-off — each run skips days already stored ok, so the runs are idempotent).
...
- **Employee share schemes** — RSU vests are entered as ordinary Buy trades in the plan [holding account](#holding-accounts) (quantity, market price at vest — the ESS cost base — and zero brokerage). Unvested grants are not tracked (they are not shares), and ESS income reporting is out of scope.
...
- **Intraday prices** — the [closing-price history](#closing-prices) stores one closing/reference price per listing per day; intraday prices are not stored. A back-dated trade does not auto-backfill price history — backfill is on demand via `POST /closing_prices/backfill`.
