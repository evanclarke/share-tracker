...
> 22 June 2026). Mirrored here are the page's **Wrapped tokens** section and
...
A wrapped token is a tokenised representation of another crypto asset. You can
typically redeem wrapped tokens (unwrapped) on a 1:1 ratio for the crypto
assets they represent (the original tokens).
...
- the value of a crypto asset of a certain standard to be represented as a
  different standard (for example, creating an ERC-20 compliant version of
  ETH).

To wrap tokens, you send crypto assets to the smart contract's address, and it
mints (creates) a wrapped version of the crypto asset. The original crypto
asset is 'locked' in the smart contract's address until the wrapped token is
redeemed.
