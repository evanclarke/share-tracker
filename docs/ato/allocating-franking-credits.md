# Allocating franking credits — the maximum franking credit

> Source: https://www.ato.gov.au/businesses-and-organisations/corporate-tax-measures-and-assurance/imputation/paying-dividends-and-other-distributions/allocating-franking-credits (QC 47305)
> Retrieved: 2026-08-17 from the Australian Taxation Office (ato.gov.au)
...
## Project note — how this drives the income write-time check

Read on the **member's** side, the last list above is the operative rule: a statement showing a
credit above the maximum does not entitle its recipient to the excess — the recipient is entitled
only to the maximum that *could* have been allocated. So a franking credit above the ceiling is
never a claimable figure for this system's taxpayer, whatever a mis-keyed row says.
...
Two scope notes on that check. It applies only to **non-trust** rows: a trust's "franked
distributions from trusts" component can be reduced by the trust's own deductions while the member
still claims the full franking credit
([`amma-statement-guidance-notes.md`](amma-statement-guidance-notes.md), Part B item 13Q), so the
ratio genuinely need not hold there. And it applies only to distributions paid on or after
**1 July 2001**, when the 30% corporate rate began; the higher imputation rates in force before
that (34%, 36%, 39%) would make the 30/70 ceiling a wrong rejection, and this project has no
pre-2001 dividend data to check it against.
