...
The CGT reform's cost base indexation, for a parcel whose expenditure was incurred on or after 1 July 2027: the cost base is indexed to the quarter the disposal happens in by that quarter's CPI over the expenditure quarter's (s 960-275(1B), worked out to 3 decimal places by s 960-275(5)), the 12-month ownership rule is the same test the discount clock uses, a capital loss is never indexed, and the indexed gain carries no discount (repealed for it). The quarterly ABS CPI series is kept current by the scheduled `cpi-import` job.

A parcel **held across 30 June 2027** is split by Subdivision 112-E instead: the deemed disposal is measured at the listing's stored 30 June 2027 closing price, its **deferred** pre-2027 gain keeps the old law's discount character and the **current** post-2027 component is indexed from the quarter beginning 1 July 2027 — both in the year of the real disposal, with the 12-month clock counting actual continuous ownership across the deemed event (EM 1.56–1.57, 1.72). Both components ride on the realised-gains and net-capital-gain parcel rows. A missing 30 June 2027 price is refused with a `422` naming the listing and the day, never defaulted to the cost base.

The reform's **seven-step net-capital-gain method statement** (new s 102-5(1), s 102-6; EM 1.75–1.106) computes the year's net capital gain from tax year **2028** — the income year that includes 1 July 2027, whose first day the commencement is, so no year mixes the two regimes. The year's gains are classified into the four categories in statutory order (deferred non-residential, deferred residential, non-residential, residential), current-year and carried-forward losses reduce them in that order, the quarantined-amount steps 3–4 apply to the residential categories, and the 50% discount falls at step 5 only on a deferred gain the old law would have discounted — a post-2027 indexed gain is never discounted. Every gain this database can hold is non-residential, so the two residential categories and the quarantined amount are **structurally nil, not omitted** (s 102-6 needs a residential dwelling and a dwelling-use record this data model has not got). The four gross categories, their post-loss remainders and `quarantined_amount` are on the net-capital-gain record and its CSV export, and the Annual Tax Report prints the seven-step layout for a reform year.

The s 114-25 residency testing period is a **recorded** answer, not an assumption: a financial year can be marked as containing a foreign or temporary residency period (Tax Year Settings), which denies indexation for a disposal whose testing period runs through it. The reform's **Division 119 30 per cent minimum tax** is **partly** computed alongside it: the year's **minimum tax capital gain** (the post-1 July 2027 non-residential gain left after the loss pool), the step-1 **30% benchmark**, and the working around the s 119-10(2) **recorded** gap — the steps 2–4 already-borne figure, the Rates Act s 12AA rate (a rate, never cent-rounded) and the extra income tax — plus the recorded s 119-15 income-support exemption, all on the net-capital-gain record and the Annual Tax Report's **Minimum tax (Division 119)** table. The **gap itself is the taxpayer's own figure**, entered per year in Tax Year Settings, because steps 2–4 need a basic income tax liability on a taxable income this tool does not compute; the exemption is recorded, never inferred. What is still not implemented — the Ministerial apportioning method and a parcel whose cost was carried from an earlier holding — is stated in [Known limitations](API.md#known-limitations), and a disposal dated on or after 1 July 2027 that the app cannot honestly assess is refused (a logged `500` naming the date) rather than assessed under repealed law.

The reform's **out-of-model assets and situations** are decided N/A rather than approximated, and recorded with what would have to change: **new residential dwellings** (new s 115-102) and **affordable housing** (s 115-125) are residential property this app holds no record of, **quarantined amounts / negative gearing** (Schedule 2 and the method statement's new steps 3–4) have no residential-dwelling income or deductions to come from, **pre-CGT holdings** and **CGT event K6** stay outside the write-time 20 September 1985 floor, a trustee's **s 115-235** beneficiary reporting is not this taxpayer, **Division 152** is expressly unchanged (no action), and the **EM 1.119 residency arm** is refused rather than approximated (see [Known limitations](API.md#known-limitations)).
...
A custom range is ad-hoc and isn't remembered) and a custom range, and a [period performance](API.md#period-performance) summary for the selected range: the period's return attributed into **capital growth**, **FX movement**, and **income** (always summing exactly to the return), plus per-holding contributions (each row carrying a **sparkline** of the security's unit price across the same window, drawn from the stored snapshots and scaled about the price the holding opened the window at (which plots on the middle line, so every row's line starts at the same height and the column can be scanned for who is up and who is down), so the shape of the move sits beside the figures that quantify it — per unit rather than per holding value, so a purchase inside the window cannot draw a rise the security did not have — green where it is above the price the holding opened at and red where it is below, cut at the crossing itself; drawn only over the part of the window the holding was actually held for — a holding bought in June and sold in July is a short line where those weeks fall, the rest of the window a faint dashed rule rather than a line stretched across it; the section's expanded state is remembered across reloads and range changes; a default-checked "hide holdings with no activity in this period" checkbox, also remembered across reloads, filters out holdings that were fully closed before the range began) and a per-currency FX breakdown
...
The overall CGT position per financial year, under the method statement that governs the year: before 1 July 2027 it combines realised parcel gains with AMMA-attributed CGT gains and capital losses, applies losses ATO-optimally (non-discountable gains first), carries unused net capital losses forward across years (seeded by an enterable opening carried-forward loss), and applies the 50% discount to produce the assessable net capital gain; from tax year 2028 it is the reform's seven-step statement over the four gain categories, with the loss pool consumed in statutory order (deferred gains before current-year gains), the quarantined-amount steps 3–4 structurally nil, and the discount only on a deferred gain the old law would have discounted — a disposal dated on or after 1 July 2027 arrives already indexed and is never discounted. Each year's CSV row carries the four categories, their post-loss remainders and the quarantined amount; expandable in the web UI from a year down to its realised disposals and, within each, its individual parcels
...
- the reform's **out-of-model assets and situations** — **new residential dwellings** (new s 115-102, the 50% discount by choice) and **affordable housing** (s 115-125, up to 60%), **quarantined amounts / negative gearing** (Schedule 2 and the method statement's new steps 3–4), **pre-CGT holdings** and **CGT event K6**'s deferred liability, a trustee's **s 115-235** beneficiary reporting, and the **EM 1.119** residency switch-off — are N/A rather than approximated: this database holds only shares, ETFs, LICs, trusts and crypto, its one taxpayer is an Australian-resident individual, and a pre-CGT acquisition is refused at write time, so each has nothing here to apply to (**Division 152** is expressly unchanged by the reform — no action). The decisions, their citations and what would have to change are collected in [Known limitations](API.md#known-limitations).
- the **CGT reform from 1 July 2027** (cost base indexation and a 30 per cent minimum tax on capital gains, replacing the 50% discount for individuals and trusts, with a 30 June 2027 deemed reacquisition and a seven-step net-capital-gain method statement) is **partly implemented**: the cost base indexation, the Subdivision 112-E boundary split and — from tax year 2028 — the seven-step net-capital-gain method statement, plus the Division 119 minimum tax's covered gain and working around the **recorded** gap, are computed (see *Cost base indexation from 1 July 2027* above and the *Net capital gain* report); the minimum tax's **steps 2–4** — the gap is the taxpayer's own figure recorded per year — the Ministerial apportioning method and a carried-cost parcel reaching a post-2027 disposal are not, and because the only thing keeping a post-2027 CGT event out of the database is the write-time `date <= today` ceiling, any report that would otherwise assess one under repealed law **refuses** it with a logged `500` naming the date rather than silently discounting it (see [Known limitations](API.md#known-limitations)).
