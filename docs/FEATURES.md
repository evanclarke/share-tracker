...
A single shared username + Argon2 password hash, set in `[auth]` in the config file, gates the whole application behind a `/login` page and a signed session cookie (access control only, not per-user data — the app stays single-taxpayer); a bearer token can be accepted alongside it for the deployment scripts that call the HTTP API without a browser. `POST /login` also carries a **bounded per-source lockout** — 5 consecutive failures from one peer IP address (the port is ignored, and an IPv6 peer is keyed on its /64, so reconnecting or rotating the interface identifier cannot evade it), then a 5-minute `429`/`Retry-After` cooldown during which even the correct password is refused; the attempt is counted at the gate, before the password is hashed, so concurrent guesses cannot race past the budget, and at most 4 password verifies ever run at once (on the blocking pool, so a flood cannot multiply Argon2's 19 MiB or stall the async workers) — because the API is a script/LLM surface as well as a browser one. Behind a reverse proxy the peer is the proxy, so the optional `[auth] trusted_proxies` list declares which peers' forwarded client address the lockout may believe (opt-in and per-address: an undeclared client's `X-Forwarded-For` is ignored, so the lockout cannot be evaded by sending a header); it is **defence in depth against brute force**, not a user account system (still one credential, no per-user state, no permanent lock, and an in-memory table a restart forgets). Off by default: the server serves exactly as it always has until `[auth]` is set (see [Authentication](API.md#authentication)).
