...
- the **CGT reform from 1 July 2027** (cost base indexation and a 30 per cent minimum tax on capital gains, replacing the 50% discount for individuals and trusts, with a 30 June 2027 deemed reacquisition and a seven-step net-capital-gain method statement) is **not implemented** — the reports compute the law in force for the years they cover, and because the only thing keeping a post-2027 CGT event out of the database is the write-time `date <= today` ceiling, every report that applies the discount now **refuses** such an event with a logged `500` naming the date rather than silently discounting it (see [Known limitations](API.md#known-limitations)).
