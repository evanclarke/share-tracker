...
Takes no parameters — the action's terms and the holdings at its `date` determine everything. With the Div 125 rollover chosen, any capital gain or loss under the demerger is disregarded, the cost base of each parcel is spread over the remaining head interests and the new demerged-entity interests by the advised percentages, the head interests' acquisition dates are unchanged, and the new interests' 12-month discount clock runs from the date the corresponding original interests were acquired (`docs/demergers.md`, the ATO's Example 32). The demerge therefore creates, in one transaction:
