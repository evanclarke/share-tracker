...
**The file is a secret and must stay owner-only** (`chmod 600`). It holds `[auth].password_hash` and `[auth].api_token`, and neither is merely a credential: `password_hash` is the direct input to the session-signing key, so anyone who can read the file can mint a valid session cookie for any expiry without knowing the password. The FreeBSD package installs the sample `0600` owned by the service user (the server runs as it, so a root-owned `0600` file would be unreadable to the server), re-tightens the live copy and its ownership on upgrade, and makes its `/var/db/share-tracker` data directory (the database and the backups beside it) `0700`; if you install it yourself, tighten both. The server logs a startup `WARN` naming a config file it loads whose group or other bits are set, but never changes the mode itself — see [Authentication](#authentication).
...
`POST /login` carries a **bounded per-source failed-login lockout**, which makes online guessing of the single shared credential impractical — not impossible, and the limits are stated below and in [docs/API.md](docs/API.md#login-lockout-defence-in-depth-not-an-account-system) rather than glossed: after **5 consecutive failed** attempts from one source, further attempts from it are refused for **5 minutes**, with a **`429 Too Many Requests`** and a `Retry-After` naming the remaining seconds, rounded up (a browser gets the same `429` with the sign-in page as the body, so the refusal is visible in an access log whatever header the client sends, and a correct password during a lockout is still refused — that is the point). The source is the request's peer **IP address** (the port is ignored deliberately — a fresh ephemeral port per connection would otherwise buy a fresh budget per guess), keyed on the **/64 prefix** for IPv6 (rotating the interface identifier inside a /64 is free, so the full address would too), and at most 4096 sources are tracked at once (least-recently-seen evicted first), so the table cannot be grown into a memory-exhaustion hole; a success before the budget clears the count, and an attempt is counted at the gate before the password is hashed, so concurrent guesses cannot exceed the budget by racing it. This is **defence in depth**, not a user account system: one shared credential, no per-user state, no permanent lock, and an in-memory table a restart forgets. It is not a config knob — the budget, window and cooldown are consts in `infra::auth`, so nothing changed in the sample config. **Behind a reverse proxy the peer is the proxy**, so every client shares one source: the lockout still bounds aggregate guessing, but one attacker locks out everybody — the **bearer token**, which the lockout does not cover, is what keeps scripted access working meanwhile. Independently of all of that, the server verifies at most **4 passwords at once** (on the blocking pool, not on an async worker), refusing an attempt that waits over 5 seconds for a slot with a `429` and `Retry-After: 1`: Argon2 is ~19 MiB and ~30 ms per verify, and a flood arriving from many sources is exactly what the per-source lockout does not price. Rate-limiting `/login` at the proxy as well (next section) is the complementary per-client control; the two are meant to be used together, because Argon2's ~30 ms/attempt is a per-guess cost, not a bound on how many guesses a source gets.

<details>
<summary>What "per source" means in each deployment</summary>

- **Direct** — the server reached on its own address: the peer is the real client, so the lockout is per client host (for IPv6, per /64, since that is the allocation a host rotates inside for free), as you would expect.
- **Behind the documented nginx proxy**: every proxied request arrives from `127.0.0.1`, so by default there is one bucket for the whole internet — the lockout protects the *credential* at the cost of letting one attacker deny sign-in to everyone. Set **`trusted_proxies = ["127.0.0.1", "::1"]`** in `[auth]` to fix that: for a request whose peer is one of those declared addresses, the lockout keys on the client address nginx forwarded, so each client gets its own budget. It is opt-in and per-address precisely because a blanket trust of `X-Forwarded-For` would be forgeable — an undeclared client's header is ignored, so nobody can choose their own bucket by sending one. Only the **rightmost** entry is read (nginx's `$proxy_add_x_forwarded_for` appends what it saw, so everything to its left is client-supplied), which trusts exactly one hop; with the setting absent nothing trusts the header at all. `limit_req` in front is still the complementary per-client control; neither replaces the other. The **bearer token** is unaffected by the login lockout, so scripted/API access keeps working from anywhere while the browser sign-in is locked out.
...
It runs via `sh -c`, so ordinary shell syntax (multiple commands, pipes, redirection) works. This is the recommended way to get an off-machine copy: because it fires exactly once per completed, verified backup, the offsite copy can never race a slow backup or silently miss a run the way a fixed-offset cron job can. A failing command fails the backup job loudly (logged at ERROR, recorded as the run's error in `GET /jobs`) but never blocks the backup itself or its local pruning. Because that error text is what `POST /jobs/backup` returns and what the Jobs screen shows, the userinfo of any URL in the command is masked before it is recorded — `curl https://user:pass@host/…` reads as `curl https://***@host/…`, so the credential never lands in the response, in `job_runs.error`, or in the log; the rest of the command is kept, since it is your own diagnostic. Credentials passed any other way (a `--user` argument, an environment variable) are not detected, so prefer an auth mechanism that reads a key file — an rclone config, an SSH key, or `--netrc-file`.
