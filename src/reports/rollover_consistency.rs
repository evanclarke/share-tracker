//! Do the three parcel-substituting operations' **stored** figures still match
//! what today's facts say they should be?
//!
...
//! `BonusIssue` dated on or before an operation that has already run, and
//! `entities::amit_adjustment` refuses an adjustment covering units one carried
//! away, so the two loudest routes into that state are closed at write time.
//! This report is the answer for the rest — a source parcel's price, brokerage
...
/// Every rollover group, oldest first.
async fn db_groups(conn: &mut sqlx::SqliteConnection) -> Result<Vec<Group>, sqlx::Error> {
    ...
    rows.iter()
        .map(|row| {
            let (kind, group_id) = if let Some(id) = row.try_get::<Option<i64>, _>("transfer_id")? {
                (RolloverKind::Transfer, id)
            } else if let Some(id) = row.try_get::<Option<i64>, _>("scrip_action_id")? {
                (RolloverKind::ScripForScrip, id)
            } else {
                let id: Option<i64> = row.try_get("demerger_action_id")?;
                (RolloverKind::Demerger, id.unwrap_or_default())
            };
