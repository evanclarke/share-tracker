...
export const REPORTS = [
    ...
    tables: [
      {
        ...
        expand: {
          ...
          columns: ['purchase_trade_id', 'acquisition_date', 'units', 'cost_base', 'proceeds', 'capital_gain_loss', 'discount_eligible', 'indexation_eligible', 'indexed_cost_base'],
        },
      },
    ],
  },
  ...
  {
    slug: 'indexation-cross-check', title: 'Indexation Cross-Check', api: '/reports/indexation_cross_check', method: 'GET',
    menu: 'Reports', section: 'Cross-checks & alerts',
    desc: 'Which of the two lawful methods — the 50% CGT discount this system applies throughout, or indexation — would have assessed less on each disposal indexation is still available for: a parcel whose cost was incurred by 21 September 1999, sold at a gain. Each row shows the CPI quarter, the factor (68.7 ÷ that quarter’s CPI, to 3 decimal places), the indexed cost base beside the one actually used, and both methods’ assessable gains. Advisory only: nothing here changes the Net Capital Gain, the Tax Summary, the Annual Tax Report or any export — taking the other method is your own adjustment, and it cannot be combined with the discount or used on a capital loss. Read each row under the qualifier its year states: the comparison is made before any capital losses, and applying losses can only move it further toward indexation.',
    tables: [
      { key: 'comparisons', title: 'Eligible disposals — discount against indexation' },
      { key: 'years', title: 'By financial year (and the losses each comparison is made before)' },
    ],
  },
  { slug: 'amit-adjustment-cross-check', title: 'AMIT Adjustment Cross-Check', api: '/reports/amit_adjustment_cross_check', method: 'GET', menu: 'Reports', section: 'Cross-checks & alerts', desc: 'AMMA statements whose per-parcel AMIT adjustments don’t reconcile to the statement: none entered at all, adjusted units outside the band the year allows — the units held at its end, plus the units disposed of during it, so a statement for the year a holding was sold reconciles rather than reading as excess coverage (split-aware) — the same parcel adjusted twice, a parcel that cannot have been held in the statement’s year, or the statement’s own units held disagreeing with the units actually open on its holding account at the year end — the one a set that reconciled when it was generated cannot notice, since entering a Buy dated before the year end moves the parcels while the statement and its set stand still. A missed parcel overstates its cost base; a duplicated one over-reduces it, and CGT event E10’s nil floor can turn that into a capital gain that was never made. Generate adjustments from the statement’s row to clear a row.' },
