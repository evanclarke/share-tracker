...
const JOB_DESC = {
  ...
  'weekly-summary': 'Email the Portfolio Overview\u2019s week: the period headline (return, capital growth, FX movement, income), the market-value and unrealised-gain series over the window, and the per-holding contributions with each security\u2019s unit price at both ends. The window is the stored report snapshots, resolved the way the Overview screen resolves a range, so the figures match the screen exactly. Sends nothing (and says so) when no [email] is configured, or when fewer than two snapshots fall in the week.',
  'price-alert': 'Email an alert for every held listing whose latest stored closing price moved more than the configured threshold (default 5%) from the previous stored close \u2014 a fall as much as a rise, in the listing\u2019s own quote currency. Scheduled after each market close; a move is alerted at least once however many runs re-find it, and a pair straddling a split or consolidation is skipped rather than reported as a crash that never happened.',
